...
pub struct Params {
    ...
    state: State,
    /// Environment variables as list of KEY=VALUE strings.
...
impl Module for Podman {
    ...
    fn exec(
        ...
        Ok((
            podman_container(parse_params(optional_params)?, check_mode)?,
            None,
        ))
    }
...
impl PodmanClient {
    ...
    fn create_container(&self, params: &Params) -> Result<bool> {
        ...
        if let Some(ref cpus) = params.cpus {
        ...
        if let Some(ref systemd) = params.systemd {
            args.push("--systemd".to_string());
            args.push(systemd.clone());
        }
    ...
    fn generate_systemd_unit(&self, name: &str) -> Result<bool> {
        if self.check_mode {
            return Ok(true);
        }
...
fn podman_container(params: Params, check_mode: bool) -> Result<ModuleResult> {
    ...
    match params.state {
        ...
        State::Present | State::Started => {
            ...
            if params.generate_systemd {
                client.generate_systemd_unit(&params.name)?;
                output_messages.push(format!(
                    "Generated systemd unit for container '{}'",
                    params.name
                ));
                changed = true;
            }
...
mod tests {
    ...
    fn test_parse_params_systemd() {
        ...
        .unwrap();
        let params: Params = parse_params(yaml).unwrap();
        assert_eq!(params.systemd, Some("always".to_string()));
        assert!(params.generate_systemd);
    }
    ...
    #[test]
    fn test_parse_params_state_present() {
        let yaml: YamlValue = serde_norway::from_str(
            r#"
            name: myapp
            image: nginx:latest
